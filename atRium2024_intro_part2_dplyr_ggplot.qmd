...
## Exercise - TBF {background-color="lightpink"} 

## Task:
- create a new project and save it to a new, parent folde. Call it for example as "atRium2024"
...
- load and/or install packages: here, dplyr
...
```{r}
df_darts %>% 
  select(Name, Catalog, Length, Width, Weight) -> df_darts_edit
```
...
ggplot(data = df_darts_edit)+
  aes(x = dart_length)+
  geom_histogram(binwidth = 2, fill = "grey75", color = "white")+
  theme_light()
```
